...
def build_color(plot=False,shuffle=True,flatten=True):
    cur_hue = start_val[0]
    hsl_list = []
    ...
    if shuffle:
        hsl_list = _determinable_shuffle(hsl_list)
    if plot:
        outname = "pallete.png"
        produce_plot(hsl_list,outname)
        return outname 
    else:
        return ColorPicker(hsl_list)

def produce_plot(hsl_list,outname):
    hsl_list = hsl_list[0:int(len(hsl_list)/4)]
...
def _determinable_shuffle(hsl_list):
    '''
    ...
    while next_row_i < num_rows:
        hsl_list = _swap_columns(hsl_list,next_row_i-1,next_row_i)
        next_row_i +=2
...
def _swap_columns(hsl_list, pos1, pos2):
