...
class AbstractDash:
    ...
    def create_button(self,identifier,href=None,add=False,**kwargs):
        if href is None:
            ...
            button = html.A(html.Button(identifier), href="/"+href, target="_blank",**kwargs)
        if add:
    ...
    def create_sidebar(self,id,name,content,add=False,**kwagrs):
        children=[html.H2(name, className="display-4"),html.Hr(),*content]
    ...
    def create_modal(self,identifier,close_identifier,name,contents,add=False,**kwargs):
        ...
        modal_footer = dbc.ModalFooter(dbc.Button("Close", id=close_identifier, className="ml-auto"))
        modal = dbc.Modal(id=identifier,children=[modal_header,modal_body,modal_footer],size="xl",**kwargs)
        if add:
    ...
    def create_question_mark(self,identifier,add=False,**kwargs):
        q_mark = html.Abbr(id=identifier,children="\uFE56",className="help-tip")
        if add:
